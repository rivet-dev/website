...
Authenticate with your admin token. See [Credentials & dashboard](/cloud/byoc/credentials) to retrieve it.

### From a public endpoint

When `rivet_endpoint_access` is `public`, use your configured hostname, such as `https://rivet.mycompany.com`. HTTPS is configured automatically after [DNS setup](/cloud/byoc/quickstart#configure-dns-for-public-access).

Authenticate with your [admin token](/cloud/byoc/credentials), or [create a public token using ACL](/actors/self-host/control-plane/rbac#publishable-key).
...
- [BYOC quickstart](/cloud/byoc/quickstart)
- [Connect workers](/actors/self-host/workers)
