...
The generated `.github/workflows/rivet-deploy.yml` deploys production on push to your default branch, creates a namespace per pull request, posts the preview link as a comment, and cleans the namespace up on close. See [Rivet Compute](/docs/deploy/cloud/compute).
...
The action also configures the Vercel Deployment Protection bypass header, which is otherwise a common source of `401`s from Rivet into preview deployments. See [Vercel](/docs/deploy/self-host/workers/vercel).
...
- [Rivet Compute](/docs/deploy/cloud/compute)
- [Tokens](/docs/deploy/cloud#tokens)
