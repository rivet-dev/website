...
This is a `cloud_api_*` management token, not a `pk_*` publishable key. See [Tokens](/docs/deploy/cloud#tokens).
...
The workflow deploys production on push to your default branch and creates an isolated namespace per pull request. See [Preview Deployments](/docs/deploy/cloud/preview-deployments).
