...
// Kept in step with src/sitemap/docs-sources.ts. Duplicated as plain data
// because this script runs under bare Node before any TS pipeline exists.
const DOCS_SOURCES = {
	actors: "rivet",
	agentos: "agentos",
...
function resolveTarget(productId, repo) {
...
for (const [productId, repo] of Object.entries(DOCS_SOURCES)) {
	const linkPath = path.join(CONTENT_BASE, productId);
	const current = describe(linkPath);
	const { target, via } = resolveTarget(productId, repo);
...
// ---------------------------------------------------------------------------
const ARTIFACTS = [
	["rivetkit-typescript/artifacts/actor-config.json", "actor-config.json"],
	["rivetkit-typescript/artifacts/registry-config.json", "registry-config.json"],
	["engine/artifacts/config-schema.json", "engine-config-schema.json"],
];

const rivetRoot = path.resolve(path.dirname(realpathSync(path.join(CONTENT_BASE, "actors"))), "..");
const artifactsDir = path.join(GENERATED, "artifacts");
mkdirSync(artifactsDir, { recursive: true });

for (const [from, to] of ARTIFACTS) {
	const src = path.join(rivetRoot, from);
	if (!existsSync(src)) {
		console.error(
			`\nassemble failed:\n  - missing ${from} in the Rivet repo (${rivetRoot}). ` +
...
console.log(`copied ${ARTIFACTS.length} config schemas from ${path.relative(REPO_ROOT, rivetRoot)}`);
