...
- **Host functions.** Registered [host functions](/agentos/docs/host-functions) are the only sanctioned way to hand the guest a named host capability. The guest invokes a function by name with JSON input, the call round-trips to the host handler, and only the handler's return value comes back. The guest never receives the underlying host access.
