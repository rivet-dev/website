...
Before reaching for logs, open the actor inspector in the Rivet dashboard. It shows the live transcript, pending permission requests (the most common reason an agent looks stuck), and the process tree.
...
It's a VM-level option covering every session's agent process; if omitted,
chunks are written to the host `process.stderr` by default. See
[Sessions](/agentos/docs/sessions) for session lifecycle and history.
...
<CodeSnippet file="examples/debugging/agent-exit.ts" />
...
The examples above already use `AgentOs.create()`. Its callbacks and sidecar
logs are process-local; the actor inspector is not available.
