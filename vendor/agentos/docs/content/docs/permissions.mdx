...
```ts
{
  ...
  env: "allow",
  binding: "allow",
  network: "deny",      // except the model-provider hosts listed below
}
```
...
- `network` is denied: guest code cannot reach the network until you opt in. The one exception is a short list of model-provider hosts (`api.anthropic.com`, `api.openai.com`, `generativelanguage.googleapis.com`, `openrouter.ai`), so agents work without a network grant. Set `network: "deny"` to close those too.
