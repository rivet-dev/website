...
const vm = agentOS({
  permissions: {
    network: {
      default: "deny",
      rules: [{ mode: "allow", operations: ["*"], patterns: ["dns://api.example.com", "tcp://api.example.com:*"] }],
    },
  },
});
```
...
Use `httpRequest()` directly on the VM handle. Preview URLs remain actor-only;
an embedded host must proxy and authenticate browser traffic itself.
