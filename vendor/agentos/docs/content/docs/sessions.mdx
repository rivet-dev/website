...
- The Rivet dashboard's Transcript tab renders the same session stream live, with a composer for prompting sessions by hand.
...
The session API is identical. Embedded callbacks are process-local, and history
only outlives `dispose()` when the VM has a configured `database`.
