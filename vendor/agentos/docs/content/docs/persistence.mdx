...
Configure a SQLite database, then recreate the VM with the same configuration.
Processes, contexts, subscriptions, and cron jobs remain VM-local.

<CodeSnippet file="examples/embedded/persistence.ts" title="persistence.ts" />
