...
Filesystem methods and mount descriptors are identical. Configure `database`
when the root filesystem must survive `dispose()`; mounts persist through their
own backends.
