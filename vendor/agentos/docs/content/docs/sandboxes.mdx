...
is exposed as [host functions](/agentos/docs/host-functions), provider-agnostic through
...
- **Host functions** expose sandbox process management as
  [host functions](/agentos/docs/host-functions). Run commands on the sandbox from the VM.
...
- `createSandboxFs`, `createSandboxHostFunctions` from `@rivet-dev/agentos-sandbox`.
- `SandboxAgent` and provider helpers (for example, `docker`) from `sandbox-agent`.
- Pass a provider as `sandbox: { provider: docker() }`. agentOS starts the
  client, mounts it at `/mnt/sandbox`, registers process host functions, and disposes
  it with the VM.
...
## Calling the mounted host functions

Write code through the filesystem, then run it inside the sandbox. Host functions are CLI
commands called through the same `exec`/`spawn` surface as any other command.
...
Pass a provider or an existing Sandbox Agent client to `AgentOs.create()`.
With an existing client, set `dispose` when the VM should own its lifecycle.

<CodeSnippet file="examples/sandbox/existing-client.ts" title="existing-client.ts" />
