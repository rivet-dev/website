...
Pass provider credentials through `env` on `vm.sessions.open()`. The host owns
the mapping between callers and VM handles.

<CodeSnippet file="examples/embedded/models-and-credentials.ts" title="models-and-credentials.ts" />
