...
Define a host-function collection with Zod input schemas and pass it to `agentOS({ hostFunctions })`. Each function becomes a CLI subcommand inside the VM.
...
When host functions are registered, CLI shims are installed at `/bin/agentos-{name}` inside the VM and the function list is injected into the agent's [system prompt](/agentos/docs/system-prompt), so keep descriptions concise to save tokens.
...
other API. Names become camelCase identifiers: `agentos-order-store list-orders`
is `orderStore.listOrders(input)`.
