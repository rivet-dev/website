...
- Add per-action permission checks on top of connection-level auth, and scope each tenant's [JWT](/docs/jwt) to its own actor so the boundary is enforced before a request reaches your code.
