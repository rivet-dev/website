...
- If you need more diagnostics, set `RIVET_LOG_LEVEL=DEBUG` for verbose logging. See [Logging](/actors/docs/logging) for more options.
...
See [Actor Statuses](/docs/statuses) for more about this status.
...
No server was available to run your actor. The cause depends on your [runtime mode](/docs/runtime-modes):
...
No server was available to run your actor. This is equivalent to `no_capacity` on the current allocation path. See the `no_capacity` section above for the causes and fixes for your [runtime mode](/docs/runtime-modes).
...
If your actors are still running old code after deploying a new version, your [versioning](/docs/versions) is likely not configured correctly.

Without versioning, Rivet has no way to distinguish old deployments from new ones. The behavior depends on your [runtime mode](/docs/runtime-modes):
...
To fix this, configure a version number in your [registry configuration](/actors/docs/registry-configuration). When a new version is deployed, Rivet will allocate new actors to the latest version and optionally drain old actors to migrate them.

## Actor status is pending

See [Actor Statuses](/docs/statuses) for more about this status.

An actor stays in "pending" status when Rivet is waiting for a server to accept it. The cause depends on your [runtime mode](/docs/runtime-modes):
