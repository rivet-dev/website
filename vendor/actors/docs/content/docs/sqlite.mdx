...
- For raw SQL schemas, use `migrations` from `rivetkit/unstable/migrations` to apply numbered migrations and track them in a dedicated schema-version table. See [upgrading actor state](/docs/versions#sqlite-recommended-for-complex-schemas).
