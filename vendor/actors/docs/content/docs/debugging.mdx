...
description: "Inspect a running Rivet Actor through its gateway endpoints and the actor inspector API, including state, connections, and workflow history."
skill: true
---

Every endpoint here is reached through the gateway using an actor ID. To find
actor IDs, list runners, or read provider config, see [Debugging](/docs/debugging)
in the platform docs. That page also defines the `$RIVET_API`, `$RIVET_NAMESPACE`,
and `$RIVET_TOKEN` shell variables used throughout the examples below.
