...
- **`serverless`** — the control plane calls an HTTP endpoint to wake runners on demand. Used by serverless platforms (Vercel, Cloudflare Workers, Freestyle, etc.). See [Runtime Modes](/docs/runtime-modes).
...
|---|---|---|---|
| `drainOnVersionUpgrade` | `bool` | `true` | When a new runner version is deployed, stop old runners and migrate their actors. See [Versions & Upgrades](/docs/versions#drain-on-version-upgrade). |
...
- [Runtime Modes](/docs/runtime-modes) — runner vs serverless behavior.
- [Versions & Upgrades](/docs/versions) — how `drainOnVersionUpgrade` interacts with version detection.
- [Limits](/actors/docs/limits) — request lifespan and drain grace period limits.
- [Debugging](/docs/debugging) — inspect a pool configuration via the API.
