...
- **Versions only go up**: Deploying a lower version number never moves allocation back to it. To undo a bad deploy, redeploy the previous code with a new, higher version number.

<Note>
Versions are not configured by default. See [Registry Configuration](/actors/docs/registry-configuration) to learn how to configure the runner version.
</Note>
...
<Warning>
The version **must** increase on every deployment. Never roll a deployment back to an older version number. New actors keep going to the runners still on the higher version, so as the rollback replaces them the set of runners accepting actors shrinks until it reaches zero and nothing can be scheduled. Allocation only recovers once every higher-version runner has disconnected and expired. To undo a bad deploy, redeploy the previous code with a new, higher version number.
</Warning>
...
- **New runner connection**: When a runner connects with a newer version number, the control plane immediately drains all older runners with the same name. This is the primary mechanism for [runner mode](/docs/runtime-modes) deployments.
- **Metadata polling** (serverless only): In [serverless mode](/docs/runtime-modes), runners periodically poll the control plane to check for newer versions and self-drain if one is found. This ensures old runners drain even if no new requests trigger a runner connection.
...
|---------|---------|-------------|---------------|
| `actor_stop_threshold` | 30m | Engine-side limit on how long each actor has to stop before being marked lost | [Engine config](/docs/deploy/self-host/control-plane) (`pegboard.actor_stop_threshold`) |
...
| `runner_lost_threshold` | 15s | Fallback detection if the runner dies without graceful shutdown | [Engine config](/docs/deploy/self-host/control-plane) (`pegboard.runner_lost_threshold`) |

Rivet has a max shutdown grace period of 30 minutes that cannot be configured.

## Related

- [Runtime Modes](/docs/runtime-modes): Serverless vs runner deployment modes
