...
The public endpoint is only required if using the [serverless runtime mode](/docs/runtime-modes#serverless) and if you have a frontend using RivetKit.
...
This flow applies to [serverless runtime mode](/docs/runtime-modes#serverless). For [runner runtime mode](/docs/runtime-modes#runner) or [clients configured to connect directly to Rivet](/actors/docs/clients/javascript), clients connect directly to Rivet and this metadata flow is not needed.
