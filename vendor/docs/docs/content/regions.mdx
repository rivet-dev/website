...
A self-hosted multi-region deployment runs a control plane in each region, all sharing one database and pub/sub layer, with each region reachable at its own hostname. See [Multi-Region](/docs/deploy/self-host/control-plane/multi-region) in the self-host docs for the topology configuration.
