...
**Start in milliseconds, not minutes.** A cold start includes scheduling the Actor, loading its state, and serving the first request: 12.3 ms p50 (27.7 ms p99) for an HTTP round trip waking a hibernated Actor, measured on FoundationDB with the Rust rivetkit SDK using an [experimental Rivet feature](https://github.com/rivet-dev/actors/pull/5772) that will be enabled by default in an upcoming update, against roughly 6 s for a Kubernetes pod. No image pull, no container boot.
