...
Railway has no CPU-target autoscaler, so replica count is a number you set rather than one that moves with load. Pick it for peak traffic, and use <SelfHostLink to="control-plane/kubernetes">Kubernetes</SelfHostLink> or <SelfHostLink to="control-plane/aws-ecs">AWS ECS</SelfHostLink> if you need it to scale on its own.
