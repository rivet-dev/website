...
- **Tune `drainGracePeriod` to cover graceful actor shutdown.** Time reserved at the end of `requestLifespan` for actors to stop gracefully before the request is forcibly closed. Default is 10 seconds from the control plane, and it cannot exceed 30 minutes; lower it for short-lived stateless actors, raise it if your actors do non-trivial cleanup or final SQLite writes. Configure it through your registry configuration.
