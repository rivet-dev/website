...
Compose has no autoscaler. Runner-mode workers are interchangeable, so `docker compose up -d --scale my-app=3` adds replicas that all join the same pool, but they share one host's CPU and memory. For capacity beyond a single host, or for autoscaling, move to <SelfHostLink to="workers/kubernetes">Kubernetes</SelfHostLink> or <SelfHostLink to="workers/aws-ecs">ECS</SelfHostLink>.
