...
Runner-mode workers are interchangeable, so instance count can be raised, or driven by Render's autoscaling on a CPU target. Target 60% CPU, matching the rest of the guides. Nothing needs re-registering; new instances dial out and appear under **Runners**. See [Render's scaling docs](https://render.com/docs/scaling).
