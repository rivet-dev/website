...
const BASE_SKILL_CONFIGS = {
	rivetkit: {
		...
		content: {
			collection: "docs",
			docId: "actors/docs/crash-course",
			fallbackDocIds: ["actors/docs"],
	...
	"rivetkit-client-javascript": {
		...
		content: {
			collection: "docs",
			docId: "actors/docs/clients/javascript",
		},
	...
	"rivetkit-client-react": {
		...
		content: {
			collection: "docs",
			docId: "actors/docs/clients/react",
		},
		includeReferences: false,
		includeOpenApi: false,
	},
	"rivetkit-client-rust": {
		...
		content: {
			collection: "docs",
			docId: "actors/docs/clients/rust",
		},
		includeReferences: false,
		includeOpenApi: false,
	},
	"rivetkit-client-swift": {
		...
		content: {
			collection: "docs",
			docId: "actors/docs/clients/swift",
		},
		includeReferences: false,
		includeOpenApi: false,
	},
	"rivetkit-client-swiftui": {
		...
		content: {
			collection: "docs",
			docId: "actors/docs/clients/swiftui",
		},
		includeReferences: false,
		includeOpenApi: false,
	},
...
// Doc ids are tried in order, so the primary id always wins over a fallback.
// Fallbacks may name a section root, which resolves to the first page beneath
// it.
function findSkillDoc<T extends { id: string }>(entries: T[], docIds: string[]) {
	for (const docId of docIds) {
		const exact = entries.find((entry) => entry.id === docId);
		if (exact) return exact;
		if (docId.includes("/")) {
			const nested = entries.find((entry) => entry.id.startsWith(`${docId}/`));
...
async function resolveContentTitle(config: SkillConfig): Promise<string> {
	...
	const doc = findSkillDoc(entries, docIds);
	if (!doc) {
