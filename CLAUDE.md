...
- **Secure Exec's mark is its own gothic wordmark** (`secure-exec-logo.svg`), filling the tile with no inset ring, because inside the ring it is illegible at product-bar size. It still rides in a tile like every other mark.
