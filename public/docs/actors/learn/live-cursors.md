...
Patterns for building live cursors, multiplayer presence, and realtime cursor sharing with RivetKit. One room actor fans cursor positions out to every connected client, keyed per room with [actor keys](/actors/docs/keys).
...
| --- | --- | --- | --- |
| `cursors` | [GitHub](https://github.com/rivet-dev/rivet/tree/main/examples/cursors) | Typed [actions](/actors/docs/actions) and [events](/actors/docs/events) over the RivetKit connection | `connState` per connection |
| `cursors-raw-websocket` | [GitHub](https://github.com/rivet-dev/rivet/tree/main/examples/cursors-raw-websocket) | Raw [`onWebSocket` handler](/actors/docs/websocket-handler) with a custom JSON message protocol | Socket map in `createVars` |
...
See [Lifecycle](/actors/docs/lifecycle) for `onDisconnect` and `createVars`.
