...
Ship your own UI tabs alongside a Rivet Actor — embedded directly in the dashboard inspector.
...
Use relative paths so the tab doesn't need to know the control plane origin or
actor id:
...
[Debugging → Inspector Endpoints](/actors/docs/debugging#inspector-endpoints).
...
The tab runs in an iframe at the control plane origin and can call any
...
- [Debugging](/actors/docs/debugging) — full inspector HTTP API
- [Actions](/actors/docs/actions) — actions your tab can invoke
- [State](/actors/docs/state) — state your tab can read
