...
Rivet Cloud issues two token types: `sk_` (secret key, server-side only) and `pk_` (public key, client-safe). For debugging, always use `sk_`. See [Endpoints](/actors/docs/general/endpoints) for more details.
...
| **Self-hosted engine** | The Rivet control plane handles authentication at the gateway level. |
