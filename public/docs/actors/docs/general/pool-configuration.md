...
- **`normal`** — runners connect to the control plane themselves (for example a long-running process started by you, Docker, or Kubernetes). The control plane does not start or stop runners.
- **`serverless`** — the control plane calls an HTTP endpoint to wake runners on demand. Used by serverless platforms (Vercel, Cloudflare Workers, Freestyle, etc.). See [Runtime Modes](/actors/docs/general/runtime-modes).
...
|---|---|---|---|
| `drainOnVersionUpgrade` | `bool` | `true` | When a new runner version is deployed, stop old runners and migrate their actors. See [Versions & Upgrades](/actors/docs/versions#drain-on-version-upgrade). |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `url` | `string` | — | HTTP endpoint the control plane calls to wake a runner. |
...
- [Runtime Modes](/actors/docs/general/runtime-modes) — runner vs serverless behavior.
- [Versions & Upgrades](/actors/docs/versions) — how `drainOnVersionUpgrade` interacts with version detection.
- [Limits](/actors/docs/limits) — request lifespan and drain grace period limits.
- [Debugging](/actors/docs/debugging) — inspect a pool configuration via the API.