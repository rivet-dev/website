...
When deploying to production, you need to configure endpoints so your backend can communicate with Rivet control plane and clients can reach your actors.

### Private Endpoint

The private endpoint tells your backend where to find the Rivet control plane.
...
The public endpoint is only required if using the [serverless runtime mode](/actors/docs/general/runtime-modes#serverless) and if you have a frontend using RivetKit.
...
This flow applies to [serverless runtime mode](/actors/docs/general/runtime-modes#serverless). For [runner runtime mode](/actors/docs/general/runtime-modes#runner) or [clients configured to connect directly to Rivet](/actors/docs/clients/javascript), clients connect directly to Rivet and this metadata flow is not needed.
...
|---------------------|---------------|-------------|
| `RIVET_ENDPOINT` | `endpoint` | Rivet control plane URL for your backend |
