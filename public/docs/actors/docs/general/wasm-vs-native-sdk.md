...
RivetKit runs your actors on a native or a WebAssembly runtime depending on your platform.
...
Cloudflare) provide the outbound WebSocket the control plane tunnel needs.

- **Cloudflare Workers**: [`@rivetkit/cloudflare-workers`](/actors/docs/quickstart/cloudflare)
- **Supabase Edge Functions**: [`@rivetkit/supabase`](/actors/docs/quickstart/supabase)
...
shim so the actor can open its tunnel to the control plane. The platform packages handle
this for you, which is why they are recommended.

## Related

- [Runtime Modes](/actors/docs/general/runtime-modes)
