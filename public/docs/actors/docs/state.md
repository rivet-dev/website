...
Rivet also provides an embedded SQLite database (`c.db`) for when your data needs to be queried, requires safe schema migrations, or grows too large to hold in memory. See [SQLite](/actors/docs/sqlite).
...
`createState` runs once when the actor is first created. On later starts, state is loaded from storage instead of recreated. See [Lifecycle](/actors/docs/lifecycle).
...
- [SQLite](/actors/docs/sqlite): raw SQL against the embedded per-actor database.
- [SQLite + Drizzle](/actors/docs/sqlite-drizzle): typed schema and query APIs.
