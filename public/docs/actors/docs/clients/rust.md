...
Connect Rust apps to Rivet Actors.
...
See the [Rust quickstart guide](/actors/docs/quickstart/rust) for getting started.
...
The Rust client is strongly typed. It shares the same action and event types as your actor, so define your actor in `src/lib.rs` and import those types from both your server and your client. There is no need to redefine the actor on the client. See [Define Your Actor](/actors/docs/quickstart/rust#define-your-actor) in the quickstart for the actor definition this page builds on.
...
- `namespace` - target namespace (defaults to the control plane's configured namespace).
- `token` - authentication token for the control plane.
...
For serverless deployments, set the endpoint to your app's `/api/rivet` URL. See [Endpoints](/actors/docs/general/endpoints) for details.
