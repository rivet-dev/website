...
For most use cases, [actions](/actors/docs/actions) and [events](/actors/docs/events) provide high-level connection handling powered by WebSockets that's easier to work with than low-level WebSockets. However, low-level handlers are required when implementing custom use cases.
...
See the [HTTP API reference](/actors/docs/http-api) for more information on WebSocket routing and authentication.
...
`onWebSocket` will trigger the `onBeforeConnect`, `onConnect`, and `onDisconnect` hooks. Read more about [lifecycle hooks](/actors/docs/lifecycle).

Open WebSockets will be listed in `c.conns`. `conn.send` and `c.broadcast` have no effect on low-level WebSocket connections. Read more about [connections](/actors/docs/connections).
...
Since `open` only fires once when the client first connects, use `c.conn.state` to store per-connection data that persists across sleep cycles. See [connections](/actors/docs/connections) for more details.
...
Connections are normally held at the gateway until the actor is ready. Pass `skipReadyWait: true` on `handle.webSocket()` to connect immediately, including while the actor is still starting or in the [sleep grace period](/actors/docs/lifecycle#shutdown-sequence). See [Skip Ready Wait](/actors/docs/clients/javascript#skip-ready-wait) for details.
