...
| Agent-to-agent communication | Custom | Built into [Rivet Actors](/agentos/docs/agent-to-agent) |
| Authentication | Set up yourself | [Documentation](/agentos/docs/authentication) |
...
All VM configuration is passed to `AgentOs.create()` as a single flat object. This is the consolidated config block to copy and adapt. The [`agentOS()` actor](/agentos/docs/quickstart) accepts the same options and layers persistence, sleep/wake, and preview URLs on top:

The top-level fields are documented inline above. See [Mounts](#mounts) and [Software](/agentos/docs/software).
...
Action timeouts and automatic sleep/wake are features of the [`agentOS()` actor](/agentos/docs/quickstart), not the core package. A core VM stays alive until you call `dispose()`. See [Persistence & Sleep](/agentos/docs/persistence) for the actor's sleep lifecycle.