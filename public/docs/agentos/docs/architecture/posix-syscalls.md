...
coreutils behind [process execution](/agentos/docs/processes) ship as WebAssembly
...
*shape*; they do not add host access. See the [Security Model](/agentos/docs/security-model)
...
- **`host_process`**: process management. Spawn a child process (argv, env, inherited stdio fds, working directory), wait for a child to exit, and related file-descriptor operations. This is what gives a WASM `sh` real [child process](/agentos/docs/processes) semantics; spawns go through the kernel process table.
...
- **`host_net`**: TCP sockets (connect, listen, send, receive) through the kernel socket table, gated by the same [network permission policy](/agentos/docs/networking) as everything else. Base WASI has no general socket API.
