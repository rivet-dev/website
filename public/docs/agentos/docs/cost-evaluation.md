...
<Note>For measured latency (cold start, warm execution, and reuse fast paths), see [Benchmarks](/agentos/docs/benchmarks). This page is about cost structure, not raw performance.</Note>
...
The denser you can safely pack agent work onto an instance, the lower your effective cost per execution. See [Resource Limits](/agentos/docs/resource-limits) for the per-VM caps that govern how densely you can pack, and [Processes & Shell](/agentos/docs/processes) for how guest processes run inside a VM.
...
When you do need a full Linux sandbox for heavier agent workloads, see [agentOS vs Sandbox](/agentos/docs/versus-sandbox) for how the two models combine.