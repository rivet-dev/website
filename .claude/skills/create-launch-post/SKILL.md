...
5. Read at least three recent concise posts in `src/content/posts/`, preferring `category: changelog` posts for similar features.
6. Resolve the canonical post URL, public docs route, GitHub repository, and Discord URL before drafting. The route preserves the complete dated post folder name. If the file is `src/content/posts/YYYY-MM-DD-<slug>/page.mdx`, its post ID is `YYYY-MM-DD-<slug>` and its URL is:
...
```text
src/content/posts/YYYY-MM-DD-<slug>/page.mdx
```
...
3. Run the narrowest available website formatting/type checks, then `pnpm build` when dependencies and checkout scope allow it.
...
```bash
POST_FILE="src/content/posts/YYYY-MM-DD-<slug>/page.mdx"
